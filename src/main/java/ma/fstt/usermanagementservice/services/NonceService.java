...
public class NonceService {
    ...
    /**
     * Stocker le nonce pour un wallet
     */
    @Transactional
    ...
    /**
     * Récupérer le nonce pour un wallet
     */
    public String getNonce(String wallet) {
        User user = userRepository.findByWallet(wallet)
                .orElseThrow(() -> new RuntimeException("User not found with wallet: " + wallet));

        // Vérifier si le nonce a expiré
        if (user.getNonce() == null ||
                user.getNonceExpiration() == null ||
                Instant.now().isAfter(user.getNonceExpiration())) {
            throw new RuntimeException("Nonce expired or not found for wallet: " + wallet);
        }
